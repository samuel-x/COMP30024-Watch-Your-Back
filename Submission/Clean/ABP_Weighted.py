...
class Player():
    ...
    _OWN_PIECE_WEIGHT: float = 0.445226
    _OPPONENT_PIECE_WEIGHT: float = -0.351372

    # Don't want to prioritize mobility over pieces, so it's much smaller.
    _OWN_MOBILITY_WEIGHT: float = 0.169186
    _OPPONENT_MOBILITY_WEIGHT: float = -0.507099

    # TODO: How to balance cohesiveness and mobility? They're opposing, in a way.
    _OWN_DIVIDED_WEIGHT: float = -0.060037 # Bad to be divided. Want to be cohesive!
    _OPPONENT_DIVIDED_WEIGHT: float = 0.052958 # Good for opponent to be divided.

    _OWN_NON_CENTRALITY_WEIGHT: float = -0.138244
    _OPPONENT_NON_CENTRALITY_WEIGHT: float = -0.116326
