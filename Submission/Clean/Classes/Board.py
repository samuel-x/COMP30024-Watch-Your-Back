...
class Board():
    ...
    def is_suicide(self, delta: Delta) -> bool:
        player_color: PlayerColor = delta.player
        adj_squares: List[Square] = self._get_adjacent_squares(delta.move_target.pos)
        for square in adj_squares:
            if square.state == SquareState.OCCUPIED and square.occupant.owner == player_color.opposite():
                opposite_square: Square = self.squares[Board._get_opposite_pos(delta.move_target.pos, square.pos)]
                if (opposite_square.state == SquareState.OCCUPIED and opposite_square.occupant.owner == player_color) or \
