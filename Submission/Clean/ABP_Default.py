    ...
    def action(self, turns) -> Union[str, None]:
        ...
        with(self._timer):
            ...
            if self._board.phase == GamePhase.PLACEMENT:
                delta_scores_list: List[Tuple[Delta, float]] = list(delta_scores.items())
                for delta, _ in delta_scores_list:
                    if self._board.is_suicide(delta):
                        del delta_scores[delta]
                        print("Deleted suicide place")
